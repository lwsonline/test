...
input double RsiMin = 25;
input double RsiMax = 75;
...
int CalculateMaxPositions(double riskPercentPerTrade, double totalRiskPercent, double accountBalance)
{
    double riskAmountPerTrade = accountBalance * riskPercentPerTrade / 100.0;
    double totalRiskAmount = accountBalance * totalRiskPercent / 100.0;
    int maxPositions = (int)(totalRiskAmount / riskAmountPerTrade);
...
double CalculateLotSize(double riskPercent, double stopLossPips, double accountBalance)
{
    ...
    return NormalizeDouble(lotSize, 2); // Round lot size to 2 decimal places
}
...
{ 
	...
	if (D1slowMA==EMPTY_VALUE) // no data yet
	  return; 
	...
	// Get the RSI 
	double rsi = iRSIMQL5(_Symbol, PERIOD_D1, RsiPeriod, PRICE_WEIGHTED,1);
	...
	if(trendM15 > 0 && trendM5 > 0 && trendM1 > 0)
	{ 
	  
		//Checking for support 
		if(iClose(_Symbol, PERIOD_D1, 0) > support)
		{ 
			//Check the RSI
			if(rsi < RsiMax) 
			{ 		  
	    		//Check the MA Indicator
				if(D1slowMA < D1fastMA && H4slowMA < H4fastMA && H1slowMA < H1fastMA)
				{
				 fBuy();
				}
			}
	...
	else if(trendM15 < 0 && trendM5 < 0 && trendM1 < 0) 
	{ 
		//Checking for resistance 
		if(iClose(_Symbol, PERIOD_D1, 0) < resistance)
		{ 
...
{ 
    ...
    double totalRiskPercent = 3.0; // Define the total risk percentage allowed across all open positions
    int maxPositions = CalculateMaxPositions(riskPercentPerTrade, totalRiskPercent, accountBalance);
    
    if (GetOrdersTotal() >= maxPositions)
    {
        return;
    }
    
    double lotSize = CalculateLotSize(riskPercentPerTrade, StopLoss, accountBalance);
    ...
    if (Execution_Mode == SYMBOL_TRADE_EXECUTION_INSTANT)
    {
    	if (dynamicSL == 1) 
		{
	        if (StopLoss > 0) SL = NormalizeDouble(Bid + StopLoss * Poin * volFactor, _Digits);
	        if (TakeProfit > 0) TP = NormalizeDouble(Bid - TakeProfit * Poin * volFactor, _Digits);
    	}
    	else
    	{
...
double GetVolatilityFactor(int period)
{
    ...
    for (int i = 0; i < limit; i++)
    {
        ...
        if ((i == 0) || (low < lowestlow)) lowestlow = low;
    }
